...
- **The browser demo's CPU half is a port and nothing checks it.** `demo/plugin.js`
  ...
  GLSL only. Change any of those and change the page by hand to match.
- No OpenFX port, no factory presets. The user guide is
...
`demo/` is the page at **downlink-demo.stoatworks-labs.com**, deployed from
`wrangler.toml` in this repo with `cf-run npx wrangler deploy` — no build step; what is
committed is what is served. `demo/vendor/` is the shared kit from
`~/Projects/infrastructure/stoatworks-backend/resolume-demo/` (plus the support footer)
and is not a place to edit. The page has no audio; AGENTS.md, "The browser demo", says
what else it is not.

Verify a deploy **by content, never by status code** — a stale page answers a cheerful 200:

    curl -s 'https://downlink-demo.stoatworks-labs.com/?cb=1' | grep -o '<title>[^<]*'
